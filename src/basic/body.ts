...
export default class Body extends Component {
  ...
  public addScript(script: Script): void {
    ...
    let scriptUrl = script.url.startsWith("http")
      ? script.url
      : manager.getAbsolutePath(script.url);
    if (script.type === "external" && script.url !== "") {
      const scriptSheet = this.children.find(
        (child) =>
          child.getTag() === "link" &&
          child.getAttribute("rel") === "scriptsheet" &&
          child.getAttribute("href") === scriptUrl
      );
      if (scriptSheet) {
      ...
      const newscript = new Component("script", {}, [
